...
class Kehadiran extends Component {
  constructor(props) {
    ...
    this.state = {
      dataKehadiran: [],
      dataKepulangan: [],
      dataIzin: [],      urlDb: sessionStorage.getItem("url"),
      isSudahPulang: "tidak",
  ...
  getAllDataKehadiran = () => {
    axios
      .get(this.state.urlDb + "/kehadiran/now")
      .then((response) => {
  ...
  getAllDataKepulangan = () => {
    axios
      .get(`${this.state.urlDb}/kehadiran/pulang/all`)
      .then((response) => {
  ...
  handleTanggalFilterChange = (e) => {
    ...
    axios
      .get(`${this.state.urlDb}/kehadiran/filter/${tanggalParam}`)
      .then((response) => {
        this.setState({ dataKepulangan: response.data });
      })
      .catch((err) => {
  ...
  render() {
    const dataHadir = this.state.dataKehadiran.map((data) => {
      ...
      return [
        ...
        <img
          ...
          src={`${this.state.urlDb}/uploads/${data.foto_masuk}`}
          alt={data.foto_masuk}
        />,
      ];
    });
    ...
    const dataPulang = this.state.dataKepulangan.map((data) => {
      ...
      return [
        ...
        <img
          className="w-24 rounded-full"
          src={`${this.state.urlDb}/uploads/${data.foto_keluar}`}
          ...
          {data.bukti !== null ? (
            <>
              <img
                ...
                src={`${this.state.urlDb}/uploads/${data.bukti}`}
                alt={data.bukti}
              />
            </>
          ) : (
